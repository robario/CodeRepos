...
JSONScriptRequest.prototype = {
    ...
    open: function(method, url, async, user, password) {
        ...
        if (   typeof method == 'undefined'
            || method == null
            || !JSONScriptRequest._RE['method'].test(method)
            )
        {
        ...
        if (   typeof url == 'undefined'
            || url == null
            || !JSONScriptRequest._RE['http_URL-custom'].test(url)
            )
        {
        ...
        if (new RegExp('^[^:]+://([^@]*)@[^/]+').test(url)) {
            ...
            if (!JSONScriptRequest._RE['userinfo'].test(userinfo)) {
        ...
        if (   user != null
            ...
            && !JSONScriptRequest._RE['username-value-custom'].test(user)
            )
        {
    ...
    setRequestHeader: function(header, value) {
        ...
        if (   typeof header == 'undefined'
            || header == null
            || !JSONScriptRequest._RE['field-name'].test(header)
            )
        {
        ...
        if (!JSONScriptRequest._RE['field-value'].test(value)) {
        ...
        if (new RegExp('^(?:accept-(?:charset|encoding)|content-length|expect|date|host|keep-alive|referer|te|trailer|transfer-encoding|upgrade)$').test(header.toLowerCase())) {
            return;
        }
        ...
        if (new RegExp('^(?:authorization|content-(?:base|location|md5|range|type|version)|de(?:lta-base|pth|stination)|etag|expect|from|if-(?:(?:un)?modified-since|range)|max-forwards|mime-version|overwrite|proxy-authorization|soapaction|timeout)$').test(header.toLowerCase()))
        {
    ...
    send: function(data) {
        ...
        if (!/^https?:\/\//.test(url)) {
            if (   typeof window.document.baseURI != 'undefined'
                && new RegExp('(.*/)[^/]*$').test(window.document.baseURI)
                )
            {
                url = RegExp.$1 + url;
            } else {
        ...
        if ( this._user != null || this._password != null) {
            ...
            url.replace(new RegExp('^[Hh][Tt]{2}[Pp][Ss]?://'), '$&' + userinfo + '@');
        }
    ...
    getResponseHeader: function(header) {
        ...
        if (!JSONScriptRequest._RE['field-name'].test(header)) {
            throw new JSONScriptRequest.SYNTAX_ERR('header: ' + header);
        }
    ...
    _readystatechange: function(readyState) {
        if (this.readyState == readyState) {
            ...
            // ActiveXObject of IE and XMLHttpRequest of Gecko.
        }
    ...
    _create_dispatcher: function() {
        this._initialize_dispatcher();
        for (this._id = 0, l = JSONScriptRequest._dispatchers.length; this._id < l; this._id++) {
    ...
    _dispatcher: function(json) {
        ...
        if (this._method == 'HEAD') {
            ...
            this.responseText = JSONScriptRequest._toSource(json).replace(/^\(|\)$/g, '');
